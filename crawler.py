...
CRAWL_LOGS_DATABASE_NAME = 'crawl_logs'
...
def crawl(num_api,
	...
	for name in places_name:
		...
		for geo in api.geo_search(query=name, granularity="city"):
			...
			break
	if CRAWL_LOGS_DATABASE_NAME not in couch:
		couch.create(CRAWL_LOGS_DATABASE_NAME)
	else:
		try:
			city_crawl_log = couch.database(CRAWL_LOGS_DATABASE_NAME).get(CITY_CRAWL_LOG_ID)
			log = city_crawl_log
		...
		try:
			...
			if max_id_list[api_id] == None:
				...
				for tweet in tweepy.Cursor(api.search, q="place:%s" % places_id[api_id], lang=lang, count=count,
					...
					# use tweet id_str as _id to prevent duplicate
					tweet_json = tweet._json
					if tweet_json is not None:
						
						try:
							id = tweet_json['id_str']
							db_data = {**tweet_json, "_id": id}
							
							couch[places_name[api_id].lower()].save(db_data)
						except Exception as e:
							print(e)
			...
			try:
				city_crawl_log = couch.database(CRAWL_LOGS_DATABASE_NAME).save(log_info)
			except Exception as e:
				print(e)
